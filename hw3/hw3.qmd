...
Make a symbolic link to the Parquet folder `labevents.parquet`:
```{bash}
ln -s ~/203b_hw_new/hw2/labevents.parquet ~/203b_hw_new/hw3/labevents.parquet
```
...
```{r}
sid_chart <- left_join(sid_chart_filtered, d_items, by = c("itemid" = "itemid")) %>%
...
labevents_tble <- arrow::open_dataset("labevents.parquet") %>%
  ...
  select(subject_id, hadm_id, itemid, storetime, valuenum) %>%
  as_tibble() %>%
  print(width = Inf)
```
