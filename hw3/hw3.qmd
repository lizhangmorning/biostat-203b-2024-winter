...
Q1_plot <- 
  ggplot(all_data) +
  geom_segment(data = filter(all_data, source == "ADT" & eventtype != "discharge"),
  ...
  geom_point(data = filter(all_data, source == "Lab"),
             aes(x = charttime, y = "Lab"), 
             shape = 3, size = 2) +

  geom_point(data = filter(all_data, source == "Procedure"),
...
Q2_plot <- ggplot(
  ...
  labs(x = "", y = "", color = "Type of Vital",
       title = str_c("Patient ", sid, " ICU stays - Vitals")) +
  guides(color = FALSE) 


ggsave("q2_plot.png", plot = Q2_plot,
...
Despite this trend, a notable peak is observed at 7 a.m.

However, it is important to note that this peak at 7 a.m. remains smaller in comparison to the peak observed during the evening hours.
...
ggplot(patients_tble, aes(x = anchor_age)) +
  ...
  labs(title = "Histogram of Anchor Age",
       x = "Anchor Age",
       y = "Frequency") +
...
```{r}
chartevents_tble <- open_dataset("chartevents.parquet") %>%
...
ins_p1 <- ggplot(mimic_icu_cohort, aes(x = los, y = insurance)) +
  geom_boxplot()+
  labs(title = "Length of ICU stay vs Insurance Type", x = "Length of ICU Stay", y = "")+
  scale_x_continuous(breaks = seq(0, max(mimic_icu_cohort$los), by = 2))
ins_p1
```
```{r}
ggplot(mimic_icu_cohort, aes(x = insurance, y = los)) +
  geom_bar(stat = "identity", color = "skyblue") +
  labs(title = "Total Length of ICU stay vs Insurance Type", x = "Insurance", y = "Total Length of ICU Stay")
```
...
ggplot(mimic_icu_cohort %>% filter(!is.na(marital_status)), aes(x = los, y = marital_status)) +
  geom_boxplot() +
  labs(title = "Length of ICU stay vs Marital Status", x = "Length of ICU Stay", y = "") +
  scale_x_continuous(breaks = seq(0, max(mimic_icu_cohort$los), by = 2))
```
```{r}
ggplot(mimic_icu_cohort %>% filter(!is.na(marital_status)), aes(x = marital_status, y = los)) +
  geom_bar(stat = "identity", color = "skyblue") +
  labs(title = "Total Length of ICU stay vs Marital Status", x = "Marital Status", y = "Total Length of ICU Stay")
```
...
```{r plot-wider, fig.width = 10, fig.height = 3}
ggplot(mimic_icu_cohort %>% filter(!is.na(gender)), aes(x = los, y = gender)) +
  geom_boxplot() +
  labs(title = "Length of ICU stay vs Gneder", x = "Length of ICU Stay", y = "") +
  scale_x_continuous(breaks = seq(0, max(mimic_icu_cohort$los), by = 2))
```
```{r}
ggplot(mimic_icu_cohort %>% filter(!is.na(gender)), aes(x = gender, y = los)) +
  geom_bar(stat = "identity", color = "skyblue") +
  labs(title = "Total Length of ICU stay vs Gender", x = "Gender", y = "Total Length of ICU Stay")
```
...
```{r}
ggplot(mimic_icu_cohort %>% filter(!is.na(intimeage)), aes(x = intimeage, y = los)) +
  geom_smooth() +
  labs(title = "Length of ICU stay vs Age at Intime", x = "Age at Intime", y = "Length of ICU Stay")
```
:::
...
```{r, fig.width = 15, fig.height = 9}
  mimic_icu_cohort%>%
  select(Bicarbonate, Chloride, Creatinine, Glucose, Potassium, Sodium, Hematocrit, `White Blood Cells`, los) %>%
  pivot_longer(cols = -los, names_to = "abbreviation", values_to = "value") %>%
  ggplot(aes(x = los, y = value, color = abbreviation, group = abbreviation)) +
  geom_point() +
  facet_grid(abbreviation ~ ., scales = "free") +
  labs(x = "Length of ICU Stay", y = "", color = "Type of Measurement",
...
```{r, fig.width = 15, fig.height = 9}
  mimic_icu_cohort%>%
  select('Heart Rate', 'Non Invasive Blood Pressure systolic', 'Non Invasive Blood Pressure diastolic', 
...
ggplot(mimic_icu_cohort, aes(x = los, y = first_careunit)) +
  geom_boxplot()+
  labs(title = "Length of ICU stay vs first_careunite", x = "Length of ICU Stay", y = "")+
  scale_x_continuous(breaks = seq(0, max(mimic_icu_cohort$los), by = 2))
```