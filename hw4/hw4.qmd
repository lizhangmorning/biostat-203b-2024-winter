...
Connect to the `d_labitems` table.
```{r}
d_labitems <- tbl(con_bq, "d_labitems") |>
   print(width = Inf)
```
...
#   
  ...
  right_join( select(icustays_tble,subject_id, stay_id, intime),
    by = "subject_id")  |>
...
chartevents_tble <- tbl(con_bq, "chartevents") %>%
  ...
  right_join(select(icustays_tble, subject_id, 
                    stay_id, intime),
             by = "stay_id"
  ) %>%
...
mimic_icu_cohort <-
  ...
  mutate(
    intimeage = year(intime) - anchor_year + anchor_age
    ) %>%
  filter(intimeage >= 18) %>%
...
mimic_icu_cohort <- mimic_icu_cohort %>%
  mutate(
    ...
    admission_location = fct_lump(admission_location, n = 4, other_level = "Other"),
...
mimic_icu_cohort <- mimic_icu_cohort %>%
  mutate(
    race = fct_collapse(race,
                        ASIAN = c("ASIAN", "ASIAN - CHINESE", "ASIAN - KOREAN", "ASIAN - ASIAN INDIAN", "ASIAN - SOUTH EAST ASIAN"),
                        BLACK = c("BLACK/AFRICAN AMERICAN", "BLACK/CAPE VERDEAN", "BLACK/AFRICAN","BLACK/CARIBBEAN ISLAND" ),
                        HISPANIC = c("HISPANIC OR LATINO", "HISPANIC/LATINO - CUBAN",
                                     "HISPANIC/LATINO - DOMINICAN", "HISPANIC/LATINO - PUERTO RICAN", 
                                     "HISPANIC/LATINO - SALVADORAN", "HISPANIC/LATINO - CENTRAL AMERICAN", 
                                     "HISPANIC/LATINO - MEXICAN", "HISPANIC/LATINO - HONDURAN", 
                                     "HISPANIC/LATINO - GUATEMALAN", "HISPANIC/LATINO - COLUMBIAN"),
                        WHITE = c("WHITE", "WHITE - BRAZILIAN", "WHITE - RUSSIAN", "WHITE - OTHER EUROPEAN", "WHITE - EASTERN EUROPEAN" ),
                        Other = c("UNKNOWN", "PATIENT DECLINED TO ANSWER", 
                                  "OTHER", "PORTUGUESE", 
                                  "NATIVE HAWAIIAN OR OTHER PACIFIC ISLANDER","UNABLE TO OBTAIN",
                                  "AMERICAN INDIAN/ALASKA NATIVE", "MULTIPLE RACE/ETHNICITY","SOUTH AMERICAN"))
  ) %>%
...
mimic_icu_cohort %>%
  select(-subject_id, -hadm_id, -stay_id, - intime, - outtime,- admittime, - dischtime,- deathtime, -admit_provider_id, - edregtime, - edouttime, - anchor_age,  - anchor_year, - anchor_year_group,) %>%
