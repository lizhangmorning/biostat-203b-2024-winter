...
ui <- fluidPage(
  ...
  column(8,
         tabsetPanel(
           ...
           #                selectizeInput("patient_id", "Patient ID", choices = NULL, options = list(maxItems = 1)),
...
server <- function(input, output) {
  # observe({
  #   updateSelectizeInput(session, "patient_id", choices = unique(dataset$subject_id))
  # })
...
#       left_join(procedures_id, by = c("icd_code" = "icd_code", "icd_version" = "icd_version")) |>
...
#       left_join(diagnoses_id, by = c("icd_code" = "icd_code", "icd_version" = "icd_version")) |>
#       select(long_title) |>
#       collect() |>
#       slice(1:3)
#   })
...
#     sub_id <- as.integer(input$patient_id)
#     chartevents_info |>
#       filter(subject_id == sub_id & itemid %in% c(220045, 220179, 220180, 220210, 223761)) |>
...
#     plot_title <- paste("Patient", sub_id, ",", gder, ",", age, "years old", ",", tolower(race))
#   })
#   
#   
