...
labevents_data <- read_csv(file_path, col_select = cols_only(columns_to_select))
str(labevents_data)
```
...
```{bash}
#| eval: false
gzip -d -k ~/mimic/hosp/labevents.csv.gz
```
...
it takes 0.1 seconds to ingest `labevents.csv` using Apache Arrow.

The number of rows and the first 10 rows of the result tibble match those in Q2.3.

Apache Arrow is a cross-language development platform for in-memory data that specifies a standardized language-independent columnar memory format for flat and hierarchical data, organized for efficient analytic operations on modern hardware. It is designed to accelerate big data processing, and it is used in many big data systems such as Spark, Hadoop, and Pandas.
...
```{r}
arrow::write_dataset(filtered_data, "labevents.parquet")
...
filtered_lab_events_parquet <- lab_events_parquet %>%
  dplyr::filter(itemid %in% c(50868, 50912, 50882))
...
The Parquet format is a columnar storage format that is optimized for reading and writing large datasets. It is designed to be efficient for both read and write operations, and it is particularly well-suited for use in big data systems such as Hadoop and Spark. The Parquet format is designed to be highly efficient for both read and write operations, and it is particularly well-suited for use in big data systems such as Hadoop and Spark.
...
DuckDB is an in-memory analytical database that is designed to be highly efficient for both read and write operations. It is particularly well-suited for use in big data systems such as Hadoop and Spark. DuckDB is designed to be highly efficient for both read and write operations, and it is particularly well-suited for use in big data systems such as Hadoop and Spark.
