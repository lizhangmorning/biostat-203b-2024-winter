...
```{bash}
#| eval: false
gzip -d -k ~/mimic/icu/chartevents.csv.gz > ./chartevents.csv
```
...
arrow::write_dataset(chartevents_dataset, "chartevents.parquet")


parquet_size_chart <- file.size("chartevents.parquet")zu
cat("Size of the Parquet file(s):", parquet_size_chart, "bytes\n")

chartevents_parquet <- arrow::open_dataset("labevents.parquet")

start_time_parquet <- Sys.time() 
filtered_chartevents_parquet <- chartevents_parquet %>%
...
num_rows_parquet <- nrow(filtered_chartevents_parquet)
cat("Number of rows in Parquet file(s):", num_rows_parquet, "\n")
```
